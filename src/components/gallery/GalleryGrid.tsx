  ...
  return (
    <div className="w-full">
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 lg:gap-2">
          {filteredItems.map((item) => (
            <Link
              key={item.id}
              href={`/gallery/${item.slug}`}
              className="group relative block aspect-[4/5] sm:aspect-square lg:aspect-[4/5] overflow-hidden bg-ink"
            >
              <PremiumImage
                assets={item.assets}
                altText={item.altText}
                dominantColor={item.dominantColors[0]}
                className="w-full h-full transform group-hover:scale-105 transition-transform duration-[1.5s] ease-out-expo"
